...
INN_KEYBOARD = InlineKeyboardMarkup()

ADMIN_KEYBOARD.add(BUTTON_GET_INFO)
CHANGE_POLE_KEYBOARD.add(BUTTON_PROF_ID).add(BUTTON_STUD_NUMBER).add(BUTTON_ROLE).add(BUTTON_CANCEL)
...
STUDENT_KEYBOARD.add(BUTTON_PROFCOME_SCHEDULE).add(BUTTON_GET_PROF_ID).add(BUTTON_GET_REASON_INFO)
...
INN_KEYBOARD.add(INLINE_BUTTON_INN).add(INLINE_BUTTON_BLANK)
