...
async def get_prof_id(message: types.Message) -> None:
    ...
    stud_info = await request_funcs.get_student_info(
        pole_name="Телеграм ID", value=message.from_user.id
    )
    ...
    if isinstance(stud_info, list):
        ...
        await bot.send_message(
            ...
            text=get_phrase("profcard_output", prof_id),
        ...
        await bot.send_message(
            chat_id=message.from_user.id,
            text=get_phrase("registration_require"),
            reply_markup=await keyboards.keyboard_choice(message.from_user.id),
        )


async def send_mat_help_blank(
    ...
    try:
        ...
        await bot.send_message(
            chat_id=callback_query.from_user.id, text=get_phrase("file_not_found")
        )


async def send_mat_support_blank(
    callback_query: types.CallbackQuery, state: FSMContext
) -> None:
...
async def get_profcome_schedule(
    message: types.Message = None, callback_query: types.CallbackQuery = None
) -> None:
...
async def obtain_institute_name(message: types.Message, state: FSMContext) -> None:
    ...
    response = await request_funcs.get_profcome_schedule(course_name=message.text)

    if response:
        await bot.send_photo(
            chat_id=message.from_user.id,
            photo=response["timetable"],
            reply_markup=await keyboards.keyboard_choice(message.from_user.id),
        )
        ...
        await bot.send_message(
            chat_id=message.from_user.id,
            text=get_phrase("schedule_not_available"),
            reply_markup=await keyboards.keyboard_choice(message.from_user.id),
        )
...
async def obtain_mat_help_reason_name(
    ...
    try:
        ...
        await bot.send_message(
            chat_id=message.from_user.id,
            text=get_phrase("not_such_reason"),
            reply_markup=await keyboards.keyboard_choice(message.from_user.id),
        )
...
async def fr_ask_quest(message: types.Message) -> None:
    ...
    await bot.send_message(
        chat_id=message.from_user.id,
        text=get_phrase("choose_question_name"),
...
async def obtain_question_name(message: types.Message, state: FSMContext) -> None:
    ...
    try:
        ...
        if message.text == "Не нашёл ответ на интересующий вопрос":
            ...
            await bot.send_message(
                chat_id=message.from_user.id, text=get_phrase(phrase)
            )

    except KeyError:
        await bot.send_message(
            chat_id=message.from_user.id,
            text=get_phrase("not_such_question"),
            reply_markup=await keyboards.keyboard_choice(message.from_user.id),
        )
        await state.finish()
...
def register_student_handlers(dp: Dispatcher) -> None:
    ...
    dp.register_callback_query_handler(
        send_mat_help_blank,
        lambda x: x.data and x.data == "get_mat_help_blank",
        state="*",
    )

    dp.register_callback_query_handler(
        send_mat_support_blank,
        lambda x: x.data and x.data == "get_mat_support_blank",
        state="*",
    )

    dp.register_callback_query_handler(
        get_profcome_schedule,
        lambda x: x.data and x.data == "get_profcome_schedule",
        state="*",
    )
    ...
    dp.register_message_handler(
        get_profcome_schedule, text=["Узнать расписание приёма документов"]
    )
    dp.register_message_handler(
    ...
    dp.register_message_handler(
        get_mat_help_docs_info, text=["Материальная помощь - перечень документов"]
    )
    dp.register_message_handler(
        obtain_mat_help_reason_name,
        content_types=["text"],
        state=GetMatHelpDocsInfoFSM.waiting_reason_name,
    )

    dp.register_message_handler(
        get_mat_support_docs_info, text=["Материальная поддержка - перечень документов"]
    )
    dp.register_message_handler(
        obtain_mat_support_reason_name,
        content_types=["text"],
    ...
    dp.register_message_handler(
        obtain_question_name,
        content_types=["text"],
        state=FrAskQuestFSM.waiting_question_name,
    )
