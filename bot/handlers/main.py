...
async def greeting(message: types.Message, state: FSMContext) -> None:
    ...
    check = await request_funcs.get_student_info(
        pole_name="Телеграм ID", value=message.from_user.id
    )
    if isinstance(check, list):
        await bot.send_message(
            chat_id=message.from_user.id,
            text=get_phrase("greeting_after_registration"),
            reply_markup=await keyboards.keyboard_choice(message.from_user.id),
        )
    else:
        await bot.send_message(
            chat_id=message.from_user.id, text=get_phrase("greeting_reg_ask")
        )
        await bot.send_message(
            chat_id=message.from_user.id,
            text=get_phrase("ask_personal_agreement"),
...
async def super_admin_help(message: types.Message, state: FSMContext):
    ...
    await bot.send_message(
        chat_id=message.from_user.id,
        text=get_phrase("superadmin_help"),
        reply_markup=await keyboards.keyboard_choice(message.from_user.id),
    )
...
async def registration(message: types.Message, state: FSMContext) -> None:
    ...
    res = await request_funcs.get_student_info(
        pole_name="Телеграм ID", value=message.from_user.id
    )
    ...
    if isinstance(res, list):
        await bot.send_message(
            chat_id=message.from_user.id,
            text=get_phrase("registration_already_passed"),
            reply_markup=await keyboards.keyboard_choice(message.from_user.id),
        )
    else:
        await bot.send_message(
            chat_id=message.from_user.id, text=get_phrase("enter_your_student_book")
        )
...
async def obtain_stud_info(message: types.Message, state: FSMContext) -> None:
    ...
    stud_info = await request_funcs.get_student_info(
        pole_name="Студенческий билет", value=stud_id
    )
    ...
    if isinstance(stud_info, list):
        if stud_info[0]["role"] in ["Admin", "SuperAdmin"]:
            await bot.send_message(
                chat_id=message.from_user.id,
                text=get_phrase("try_of_admin_registration_mistake"),
                reply_markup=keyboards.REGISTRATION_KEYBOARD,
            )
            await state.finish()
        else:
            ...
            student_fio = " ".join(
                [
                    stud_info[0]["surname"],
                    stud_info[0]["name"],
                    stud_info[0]["second_name"],
                ]
            )  # возможна ошибка
            await bot.send_message(
                chat_id=message.from_user.id,
                text=get_phrase("registration_confirm_require", student_fio),
        ...
        await bot.send_message(
            chat_id=message.from_user.id, text=get_phrase("input_mistake_phrase")
        )
...
async def obtain_confirm(message: types.Message, state: FSMContext) -> None:
    ...
    if message.text.lower() == "да":
        ...
        response = await request_funcs.redact_student_info(
            bd_id=bd_id, pole_name="Телеграм ID", new_value=telegram_id
        )

        if response:
            await bot.send_message(
                chat_id=message.from_user.id,
                text=get_phrase("registration_passed"),
                reply_markup=await keyboards.keyboard_choice(message.from_user.id),
            )
        else:
            await bot.send_message(
                chat_id=message.from_user.id,
                text=get_phrase("registration_passing_mistake"),
                reply_markup=keyboards.REGISTRATION_KEYBOARD,
            )
...
def register_main_handlers(dp: Dispatcher) -> None:
    ...
    dp.register_message_handler(
        cancel_handler, lambda x: x.text in ["Отмена", "В главное меню"], state="*"
    )
...
async def wtf_answering(message: types.Message, state: FSMContext) -> None:
    ...
    await bot.send_message(
        chat_id=message.from_user.id,
        text=get_phrase("not_understand"),
        reply_markup=await keyboards.keyboard_choice(message.from_user.id),
    )
