...
Причина получения МП: {student['MP_case']}
            ...
            await bot.send_message(message.from_user.id, text='_', reply_markup=InlineKeyboardMarkup().
                                   add(InlineKeyboardButton(text='Редактировать', callback_data=f'redact {student["id"]}')))
        await GetStudentInfoFSM.next()
    else:
...
async def redact_student_info(callback_query: types.CallbackQuery, state: FSMContext) -> None:
    ...
    await GetStudentInfoFSM.next()
    await bot.send_message(callback_query.from_user.id, 'Выберите поле, в которое хотите внести изменения', reply_markup=keyboards.CHANGE_POLE_KEYBOARD)
...
async def obtain_confirm(message: types.Message, state: FSMContext) -> None:
    ...
    if message.text == 'Да':
        async with state.proxy() as data:
            await request_funcs.redact_student_info(data['id'], data['pole_name'], data['new_value'])
...
def register_admin_handlers(dp: Dispatcher) -> None:
    ...
    dp.register_callback_query_handler(redact_student_info, lambda x: x.data and x.data.startswith('redact '), state=GetStudentInfoFSM.redact_student_info)
    dp.register_message_handler(obtain_change_pole, content_types=['text'], state=GetStudentInfoFSM.waiting_change_pole)
    dp.register_message_handler(obtain_new_value, content_types=['text'], state=GetStudentInfoFSM.waiting_new_value)
    dp.register_message_handler(obtain_confirm, content_types=['text'], state=GetStudentInfoFSM.waiting_confirm)
